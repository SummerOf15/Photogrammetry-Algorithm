...
class rigid_registration(object):
  def __init__(self, X, Y, R=None, t=None, s=1, sigma2=None, maxIterations=20, tolerance=0.001, w=0.5):
    if X.shape[1] != Y.shape[1]:
        print("%d,%d"%(X.shape[1],Y.shape[1]))
...
#         callback(iteration=self.iteration, error=self.err, X=self.X, Y=self.TY)
  ...
  def updateTransform(self):
    ...
    self.s = np.trace(np.dot(np.transpose(self.A), self.R)) / self.YPY[0,0]
  ...
  def updateVariance(self):
    ...
    xPx      = np.dot(self.Pt1, np.sum(np.multiply(self.XX, self.XX), axis =1))
  ...
  def initialize(self):
    self.TY = self.s * np.dot(self.Y, np.transpose(self.R)) + self.t;
    if not self.sigma2:
        self.sigma2=0;
        for i in range(self.X.shape[0]):
            XX=np.tile(self.X[i],(self.M, 1));
            diff = XX - self.Y;
            err  = np.multiply(diff, diff)
            self.sigma2 += np.sum(err) / (self.D * self.M)
...
#             self.sigma2 = np.sum(err) / (self.D * self.M * self.N)
  ...
  def EStep(self):
    ...
    for i in range(0, self.M):
        ...
        P[i, :] = P[i, :] + np.mat(np.sum(diff, axis=1)).T

    c = (2 * np.pi * self.sigma2) ** (int(self.D / 2))
    c = c * self.w / (1 - self.w)
    c = c * self.M / self.N
