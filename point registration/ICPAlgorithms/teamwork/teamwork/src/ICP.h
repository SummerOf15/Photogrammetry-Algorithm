...
namespace gs
{
	...
	*/
	...
	inline void scaleTransformation(std::vector<Point*> &dynamicPointCloud, gs::Point* staticMid, gs::Point* dynamicMid)
	{
		float scale1 = staticMid->pos[0] / dynamicMid->pos[0];
		float scale2 = staticMid->pos[1] / dynamicMid->pos[1];
		float scale3 = staticMid->pos[2] / dynamicMid->pos[2];
		for (int i = 0; i < dynamicPointCloud.size(); i++)
		{
			dynamicPointCloud[i]->pos[0] *= scale1;
			dynamicPointCloud[i]->pos[1] *= scale2;
			dynamicPointCloud[i]->pos[2] *= scale3;
		}
	}
	...
	inline void inverseTranslate(gs::Point* p, float* translationVector, gs::Point* result)
	{
		result->pos[0] = p->pos[0] - translationVector[0];
		result->pos[1] = p->pos[1] - translationVector[1];
		result->pos[2] = p->pos[2] - translationVector[2];
	...
	inline double detMatrix(float* result)
	{
		double temp1 = result[0] * result[4] * result[8] + result[1] * result[5] * result[6] * +result[2] * result[3] * result[7];
		double temp2 = result[0] * result[5] * result[7] + result[1] * result[3] * result[8] * +result[2] * result[4] * result[6];
		return temp1 - temp2;
	}
	inline void matrixMultSub(float* a, float* b, float* result)
	{
		result[0] = a[0] * b[0] + a[1] * b[3] + a[2] * b[6];
		result[1] = a[0] * b[1] + a[1] * b[4] + a[2] * b[7];
		result[2] = a[0] * b[2] + a[1] * b[5] + a[2] * b[8];
